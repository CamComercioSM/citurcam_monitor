...
//    console.log(audioElementBeep);
        if (timbreEncontradoCargado) {
                if (contadorTimbre < maximoTimbres) {
                        if (typeof audioElementBeep !== "undefined") {
                                console.log("reproducciendo el timbre de turno encontrado.")
                                if (audioElementBeep?.play()) {
                                        audioElementBeep.volumen = 1;
                                        audioElementBeep.muted = false;
                        ...
                        if (typeof audioElementBeep !== "undefined") {
                                if (audioElementBeep?.pause()) {
                                        audioElementBeep.pause();
                                }
                        }
                }
        }
...
var TIEMPO_PAUSA_PASOS = 30000;
var VELOCIDAD_CARRUSEL = 1500;
var TIEMPO_CONSULTA = 140000;
...
function cargarSedes() {
        conexionAPI('datosSedesConZonasAtencion')
                .then(data => {
                        cargarDatosZonasAtencion(data.DATOS);
//                        console.log(data); // JSON data parsed by `data.json()` call
                });
}
function _cargarSedes() {
        bloqueoCargando();
        var datosConsulta = [];
        ApiSicam.ejecutar(
...
function recibirDatosYMostrarTabla(datos) {
        if (datos) {
                ...
                }, TIEMPO_CONSULTA * Math.random() + TIEMPO_PAUSA_PASOS);
...
function mostrarTurnosPendientesTiposServicios() {
        conexionAPI('mostrarTotalTurnosPendientesPorZonasPorTiposServicios', ZonasAtencion)
                .then(data => {
//                        console.log(data); // JSON data parsed by `data.json()` call
                        recibirDatosYArrancarCarrusel(data.DATOS);
                });
}
function _mostrarTurnosPendientesTiposServicios() {
        var datosConsulta = ZonasAtencion;
        ApiSicam.ejecutar(
...
////console.log('leyendo datos desde el servidor  de turnos pendientes');
                $.each(datos, function (i, datos) {
                        ...
                        TiposServicios["pendientes"] = datos.turnosPENDIENTES;
                        ...
                        $("#carousel").append(
                                ...
                                + '<div id="tiposervicio-turno-' + datos.turnoTipoServicioID + '" class="col-xs-6 nombre-turno " >' + datos.turnosPENDIENTES + '</div>'
                                + '</div>'
                                );
                });
...
function buscarDatosTurnosPendientesTiposServicios() {
        conexionAPI('mostrarTotalTurnosPendientesPorZonasPorTiposServicios', ZonasAtencion)
                .then(data => {
//                        console.log(data); // JSON data parsed by `data.json()` call
                        cargarDatosTurnoPendientesEnCarrusel(data.DATOS);
                });
}

function _buscarDatosTurnosPendientesTiposServicios() {
        var datosConsulta = ZonasAtencion;
        ApiSicam.ejecutar(
                'tienda-apps/TurnosApp/mostrarTurnosPendientesTiposServicios',
                datosConsulta,
...
//        ////console.log('leyendo datos desde el servidor  de turnos pendientes');
        ...
        setTimeout(buscarDatosTurnosPendientesTiposServicios, TIEMPO_CONSULTA * Math.random() + TIEMPO_PAUSA_PASOS);
        return true;
}
...
//            ////console.log(PendientesTiposServicios[i]);
        ...
        let tiempo = TIEMPO_CONSULTA * Math.random() + TIEMPO_PAUSA_PASOS;
//    console.log( "mostrarDatosTurnoPendientesEnCarrusel" + tiempo )
        setTimeout(mostrarDatosTurnoPendientesEnCarrusel, tiempo);
}
...
function mostrarDatosTurnoAtendiendoEnCarrusel() {
        ...
        let tiempo = TIEMPO_CONSULTA * Math.random() + TIEMPO_PAUSA_PASOS;
//    console.log( "mostrarDatosTurnoAtendiendoEnCarrusel" + tiempo )
        setTimeout(mostrarDatosTurnoAtendiendoEnCarrusel, tiempo);
}
//Llamados cuando el monitor ya arrancó
function buscarTurnosLLamando() {

        conexionAPI('mostrarTurnosLlamandoZonasAtencion', ZonasAtencion)
                .then(data => {
//                        console.log(data); // JSON data parsed by `data.json()` call
                        recibirDatosTurnoLlamandoyCargar(data.DATOS);
                });

}
function _buscarTurnosLLamando() {
        var datosConsulta = ZonasAtencion;
        ApiSicam.ejecutar(
...
function buscarTurnosAtendiendo() {
        conexionAPI('atendiendoPorModulosPorZonasAtencion', ZonasAtencion)
                .then(data => {
//                        console.log(data); // JSON data parsed by `data.json()` call
                        recibirDatosTurnoAtendiendoyCargar(data.DATOS);
                });

}
function _buscarTurnosAtendiendo() {
        var datosConsulta = ZonasAtencion;
        ApiSicam.ejecutar(
        ...
        if (Turnos && Turnos.length) {
                ...
                if (TurnosRecibidos.length) {
                        for (var i in TurnosRecibidos) {
                                ...
                                if (TurnosRecibidos[i].turnoCODIGOCORTO !== "null") {
                                        var Turno = [];
                                        Turno["id"] = TurnosRecibidos[i].turnoID;
                                        Turno["codigo"] = TurnosRecibidos[i].turnoCODIGOCORTO;
                                        Turno["nombre"] = TurnosRecibidos[i].personaNOMBRES;
...
////console.log( TurnosAtendiendo );               
        ...
        setTimeout(buscarTurnosAtendiendo, TIEMPO_CONSULTA * Math.random() + TIEMPO_PAUSA_PASOS);
}
...
//            reproductoresVOZ[datos.id].setAttribute('muted', 'muted');
                        reproductoresVOZ[datos.id].muted = true;
//            reproductoresVOZ[datos.id].setAttribute('volumen', 0);
...
function reiniciarMonitorTurnos() {
        hablar("Se está reiniciando el monitor. Espera mientras cargamos de nuevo!!!!");
        bloqueoCargando();
        setTimeout(function () {
...
function reproducirAudio(MEDIA, volumen = 1) {
...
function abrirVideoSaludoCorportivo() {
        $.ajax({
                url: "https://monitor.citurcam.com/html/reproductor.php"
        }).done(function (data) {
                $("#saludo").html(data);
        });
}